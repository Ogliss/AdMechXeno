...
namespace AdeptusMechanicus
{
    ...
    public class JobGiver_Necron : ThinkNode_JobGiver
    {
        ...
        //    pawn.mindState.mentalStateHandler.TryStartMentalState(OGChaosDeamonDefOf.MentalState_OGChaosDeamon, null, false, false, null, false);
            ...
            if (pawn2 != null)
            {
                using PawnPath pawnPath = pawn.Map.pathFinder.FindPathNow(pawn.Position, pawn2.Position, pawn, peMode: PathEndMode.OnCell);
                if (!pawnPath.Found)
                {
                    return null;
                }
        ...
        private Job MeleeAttackJob(Thing target)
        {
            ...
            Rand.PushState();
            return new Job(JobDefOf.AttackMelee, expiryInterval)
            {
